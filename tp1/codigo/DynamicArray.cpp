...
template < class T >
bool
DynamicArray< T >::increaseLength(size_t const newLength)
{
	bool status = true;
	
	if(newLength <= allocMemorySize_)
	{
		length_ = newLength;
		return status;
	}
	else{
		while(newLength > allocMemorySize_)
			if( ( status = resize() ) == false )
				return status;
...
{	
	...
	{ // Evita la autoasignación
		...
		cerr<<"array=array is forbidden use.";	
